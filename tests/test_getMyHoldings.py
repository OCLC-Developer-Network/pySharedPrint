...
def test_getMyLibraryHoldingsOCLCNumber(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcNumber = "70775700"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings?oclcNumber=' + oclcNumber, status_code=200, json=my_holdings_oclcnumber)
...
def test_getMyLibraryHoldingsOCLCNumberSerial(requests_mock, mockOAuthSession, getTestConfig):
    getTestConfig.update({'oauth-session': mockOAuthSession})
    oclcNumber = "2445677"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings?oclcNumber=' + oclcNumber, status_code=200, json=my_holdings_oclcnumber_serial)
    holdings = make_requests.getMyLibraryHoldings(getTestConfig, "oclcnumber", oclcNumber)
    assert type(holdings) is pandas.core.series.Series
...
def test_getMyLibraryHoldingsBarcode(requests_mock, mockOAuthSession, getTestConfig):
    getTestConfig.update({'oauth-session': mockOAuthSession})
    barcode = "CR963528"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings?barcode=' + barcode, status_code=200, json=my_holdings_barcode)
...
def test_getMyLibraryHoldingsAccessionNumber(requests_mock, mockOAuthSession, getTestConfig):
    ...
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings/' + accession_number, status_code=200, json=my_holdings)
...
def test_getMyLibraryHoldingsAccessionNumberSerial(requests_mock, mockOAuthSession, getTestConfig):
    ...
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings/' + accession_number, status_code=200, json=my_holdings_serial)
    holdings = make_requests.getMyLibraryHoldings(getTestConfig, "accession_number", accession_number);
    assert type(holdings) is pandas.core.series.Series
    assert holdings[0] == "2445677"
...
def test_getMyLibraryHoldingsAccessionNumberSerial2(requests_mock, mockOAuthSession, getTestConfig):
    ...
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings/' + accession_number, status_code=200, json=my_holdings_serial2)
    holdings = make_requests.getMyLibraryHoldings(getTestConfig, "accession_number", accession_number);
    assert type(holdings) is pandas.core.series.Series
    assert holdings[0] == "2445677"
...
def test_getMyLibraryHoldings_None(requests_mock, mockOAuthSession, getTestConfig):
    getTestConfig.update({'oauth-session': mockOAuthSession})
    barcode = "CR963528"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings?barcode=' + barcode, status_code=200, json=my_holdings_none)
    holdings = make_requests.getMyLibraryHoldings(getTestConfig, "barcode", barcode);
    assert type(holdings) is pandas.core.series.Series
    assert holdings[0] == ""
    assert holdings[1] == ""
...
def test_getMyLibraryHoldingsAccessionNumber_notFound(requests_mock, mockOAuthSession, getTestConfig):
    getTestConfig.update({'oauth-session': mockOAuthSession})
    accession_number = "1"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/my-holdings/' + accession_number, status_code=200, json=my_holdings_notfound)
    holdings = make_requests.getMyLibraryHoldings(getTestConfig, "accession_number", accession_number);
    assert type(holdings) is pandas.core.series.Series
    assert holdings[0] == ""
    assert holdings[1] == ""
