...
def test_getRetainedHoldings(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcNumber = "1752384"
    group = "ASRL"
    requests_mock.register_uri('GET', 'https://americas.api.oclc.org/discovery/worldcat/v1/bibs-retained-holdings?oclcNumber=' + oclcNumber, status_code=200, json=retained_holdings_list)
...
def test_getRetainedHoldings_HeldByGroup(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcNumber = "1752384"
    group = "ASRL"
    requests_mock.register_uri('GET', 'https://americas.api.oclc.org/discovery/worldcat/v1/bibs-retained-holdings?oclcNumber=' + oclcNumber + '&heldByGroup=' + group, status_code=200, json=retained_holdings_group)
...
def test_getRetainedHoldings_HeldInState(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcNumber = "1752384"
    state = "GA"
    requests_mock.register_uri('GET', 'https://americas.api.oclc.org/discovery/worldcat/v1/bibs-retained-holdings?oclcNumber=' + oclcNumber + '&heldInState=' + state, status_code=200, json=retained_holdings_state)
...
def test_getRetainedHoldings_NoResults(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcNumber = "2416076"
    state = "CA"
    requests_mock.register_uri('GET', 'https://americas.api.oclc.org/discovery/worldcat/v1/bibs-retained-holdings?oclcNumber=' + oclcNumber + '&heldInState=' + state, status_code=200, json=no_retained_holdings)
    bib = make_requests.getRetainedHoldings(getTestConfig, oclcNumber, heldInState=state);
    assert type(bib) is pandas.core.series.Series
