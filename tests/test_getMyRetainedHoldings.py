...
def test_getMyLibraryRetainedHoldingsOCLCNumber(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcSymbol = "OCWMS"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/retained-holdings?oclcNumber=' + oclcNumber + "&heldBy=" + oclcSymbol, status_code=200, json=my_retained_holdings_oclcnumber)
...
def test_getMyLibraryRetainedHoldingsOCLCNumberSerial(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcSymbol = "OCWMS"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/retained-holdings?oclcNumber=' + oclcNumber + "&heldBy=" + oclcSymbol, status_code=200, json=my_retained_holdings_serial)
    holdings = make_requests.getMyLibraryRetainedHoldings(getTestConfig, oclcSymbol, {"type": "oclcnumber", "value": oclcNumber})
    assert type(holdings) is pandas.core.series.Series
...
def test_getMyLibraryRetainedHoldingsBarcode(requests_mock, mockOAuthSession, getTestConfig):
    ...
    oclcSymbol = "OCWMS"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/retained-holdings?barcode=' + barcode + "&heldBy=" + oclcSymbol, status_code=200, json=my_retained_holdings_barcode)
...
def test_getMyLibraryRetainedHoldings_None(requests_mock, mockOAuthSession, getTestConfig):
    ...
    barcode = "CR963528"
    oclcSymbol = "OCWMS"
    requests_mock.register_uri('GET', 'https://americas.discovery.api.oclc.org/worldcat/v2/retained-holdings?barcode=' + barcode + "&heldBy=" + oclcSymbol, status_code=200, json=my_retainedholdings_none)
    holdings = make_requests.getMyLibraryRetainedHoldings(getTestConfig, oclcSymbol, {"type": "barcode", "value": barcode});
    assert type(holdings) is pandas.core.series.Series
    assert holdings[0] == ""
    assert holdings[1] == ""
